...
int main(int argc, char *argv[])
{
  ...
  while(localDiff>epsilon){
    ...
    //	printf("%4.4f ",gplate2[row][col]);
    //  }
    //  printf("\n");
    //}

    //}
    MPI_Gather(gplate2[1], cols, MPI_DOUBLE, passrow, cols,MPI_DOUBLE, 0, MPI_COMM_WORLD);
    if(myrank==0){
      //for (int i=0;i<cols;i++)
      //rowscatter[(numranks-1)*cols+i]=passrow[i]; //gplate1[2][i]; TOP
      for (int i=0;i<cols*(numranks-1);i++)
	rowscatter[i]=passrow[cols+i];
      for (int i=cols*(numranks-1); i<cols*numranks;i++)
	rowscatter[i]=bottom;
      
    }
    MPI_Scatter(rowscatter, cols, MPI_DOUBLE,gplate2[rankrows-1],cols,MPI_DOUBLE,0,MPI_COMM_WORLD);
    MPI_Gather(gplate2[rankrows-2], cols, MPI_DOUBLE, passrow, cols,MPI_DOUBLE, 0, MPI_COMM_WORLD);
    if(myrank==0){
      
      for (int i=0;i<2*cols;i++)
	rowscatter[i]=top;
      rowscatter[cols]=myleft;
      rowscatter[2*cols-1]=myright;
      for (int i=2*cols;i<cols*numranks;i++)
	rowscatter[i]=passrow[cols-i];
      //for (int i=(cols-1)*numranks; i<cols*numranks;i++)
      //	rowscatter[i]=bottom;
      //
    }
    MPI_Scatter(rowscatter, cols, MPI_DOUBLE,gplate2[0],cols,MPI_DOUBLE,0,MPI_COMM_WORLD);

    //if(myrank==1){
    //  printf("\n-------------------------\n)");
    //for (int row =1; row<rankrows; row++){
    //  for (int col=0;col<cols;col++){
    //	printf("%4.4f ",gplate2[row][col]);
    //  }
    //  printf("\n");
    ...
    //localDiff=epsilon;
    //break;
    //printf("%d: before localDiff: %4.4f\n",myrank,localDiff);
    //break;
    //share rows

    
    //if((myrank+1)%2==0 && numranks!=2){
    //	//myrank*10;
